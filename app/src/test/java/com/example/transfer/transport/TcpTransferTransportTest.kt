...
class TcpTransferTransportTest {
    ...
    @Test
    fun `test send flushes BufferedOutputStream immediately for small messages`() =
        runBlocking {
    ...
    @Test
    fun `test handshake messages are flushed immediately across transport`() =
        runBlocking {
            val server = ServerSocket(0)
            serverSocket = server
            val port = server.localPort
    ...
    @Test
    fun `test TCP connection failure cleans up resources`() =
        runBlocking {
    ...
    @Test
    fun `test reconnect and disconnect lifecycle cleans up and restarts read loop`() =
        runBlocking {
    ...
    @Test
    fun `test concurrent send calls do not interleave or corrupt framing`() =
        runBlocking {
            val server = ServerSocket(0)
            serverSocket = server
            val port = server.localPort
