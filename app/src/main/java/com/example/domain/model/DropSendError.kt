...
sealed class DropSendError(
    ...
    class DiscoveryFailed(
        reason: String = "Unable to start local discovery",
    ) : DropSendError(
    ...
    class AuthenticationFailed(
        detail: String = "Cryptographic authentication with peer failed",
    ) : DropSendError(
    ...
    class StorageFull(
        ...
        availableBytes: Long,
            ...
            userMessage = "Not enough free storage space to receive files (${formatFileSize(
                requiredBytes,
            )} needed, ${formatFileSize(availableBytes)} available).",
    ...
    class TransferCancelled(
        val isPeerInitiated: Boolean = false,
    ) : DropSendError(
